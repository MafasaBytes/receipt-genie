...
def build_llm_prompt(ocr_text: str, rag_examples_block: str = "") -> str:
    """Build the LLM prompt from YAML config with safe fallbacks.

    When ``rag_examples_block`` is non-empty it is inserted between the
    extraction guidelines and the actual OCR text, giving the model
    concrete examples of correct output for similar receipts.
    """
    ...
    if rag_examples_block and rag_examples_block.strip():
        prompt_parts.append(rag_examples_block.strip())
...
def extract_fields_llm(ocr_text: str, rag_examples_block: str = "") -> Dict[str, Any]:
    """
    Extract structured receipt fields using Ollama LLM.

    When ``rag_examples_block`` is provided (non-empty string), it is injected
    into the prompt between the extraction guidelines and the OCR text so the
    model can use similar past receipts as few-shot guidance.

    Args:
        ocr_text: Raw OCR text from receipt
        rag_examples_block: Pre-formatted few-shot examples from the RAG pipeline

    Returns:
        Dictionary with extracted fields
    """
    prompt = build_llm_prompt(ocr_text, rag_examples_block=rag_examples_block)
